...
class Chessboard(Grid):
    def __init__(self, m, n, s_width=1, s_length=1, **kwargs):
        ...
        for x in range(0, m, 2):
            for y in range(0, n):
                pos = s_width * \
                    np.array([x - m/2 + (1 if y % 2 == 0 else 1/3)
                              * s_width, y - n/2 + s_width/3, 0])
                rect = Rectangle(
                    width=s_length,
                    height=s_width,
                    fill_opacity=0.6,
...
class DominoGrid(Grid):
    def __init__(self, m, n, s_width=1, s_length=1, dt=0.6, **kwargs):
        Grid.__init__(self, m, n, s_width=s_width, s_length=s_length, **kwargs)
        for x in range(0, m, 2):
            for y in range(0, n):
                rect = Rectangle(width=(2*s_width-dt), height=s_width-dt, fill_opacity=1, stroke_color=WHITE, color=PURPLE).shift(
                    [s_width * np.array([x - m/2 + s_length/1.5, y - n/2 + s_width/3, 0])])
                self.add(rect)
